...
func (r *run) runAudit(out *string, moduleDone, stop *chan bool) (err error) {
	...
	libaudit.GetAuditMessages(r.netlinkSocket, messageHandler, &errChan, stop)
...
func (r *run) setConfigParams() (err error) {
	...
	err = libaudit.AuditSetPid(r.netlinkSocket, uint32(syscall.Getpid()))
	if err != nil {
		return err
	}
...
func messageHandler(event *libaudit.AuditEvent, errChan chan error, args ...interface{}) {
	select {
	case err := <-errChan:
