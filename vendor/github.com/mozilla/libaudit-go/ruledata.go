...
func AuditSyscallToName(syscall string, prefixPath string) (name string, err error) {
	path, err := filepath.Abs(prefixPath + "/vendor/github.com/mozilla/libaudit-go/audit_x64_i2s.json")
	content, err := ioutil.ReadFile(path)
	if err != nil {
		return "", err
	}
	var x64Map interface{}
	err = json.Unmarshal([]byte(content), &x64Map)
	if err != nil {
		return "", err
	}
